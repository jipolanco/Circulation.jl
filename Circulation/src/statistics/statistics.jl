...
"""
    analyse!(stats::Dict, gp::ParamsGP, data_dir_base;
             ...
             max_slices = typemax(Int), resampling_factor::Int = 1,
             to = TimerOutput())
...
- `resampling_factor`: if greater than zero, the loaded ψ fields are resampled
...
function analyse!(stats::StatsDict, orientation::Val, gp::ParamsGP{D},
    ...
    @assert Nslices >= 1
    @assert resampling_factor >= 1  # only upscaling is allowed
