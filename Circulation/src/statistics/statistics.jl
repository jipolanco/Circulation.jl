...
"""
    analyse!(stats::Dict, gp::ParamsGP, data_dir_base;
             ...
             max_slices = typemax(Int), resampling_power::Int = 0,
             to = TimerOutput())
...
- `resampling_power`: if greater than zero, the loaded ψ fields are resampled into
  a finer grid using padding in Fourier space.
  The number of grid points is increased by a factor `2^resampling_power` in
  every direction.
...
function analyse!(stats::StatsDict, orientation::Val, gp::ParamsGP{D},
    ...
    Nij_compute = Nij_input .* 2^resampling_power
...
function analyse_slice!(
    ...
    gp_slice = ParamsGP(gp_slice_in, dims=size(F.ψ))
